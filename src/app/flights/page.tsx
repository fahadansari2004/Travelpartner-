...
export default function FlightsPage() {
  ...
  const handleFlightSubmit = async (e: React.FormEvent) => {
    ...
    const totalFare = activeFlightModal.farePrice ? activeFlightModal.farePrice * seatsRequested : 0;
    const fareText = totalFare > 0 ? `₹${totalFare.toLocaleString("en-IN")}` : "Best Price / Price on Request";
              ...
              <form className="space-y-4 text-xs" onSubmit={handleFlightSubmit}>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/10">
                  <div>
                    ...
                    <span className="text-2xl font-bold text-amber-400">
                      {activeFlightModal.farePrice ? `₹${(activeFlightModal.farePrice * seatsRequested).toLocaleString("en-IN")}` : "Price on Request"}
                    </span>
                  </div>
