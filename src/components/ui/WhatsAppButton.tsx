...
import { useStoreData, INITIAL_CONTACT, ContactSettings } from "@/lib/storage";
...
export function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-auto">
      ...
      {isOpen && (
        <div className="mb-3 w-72 sm:w-80 glass-card bg-slate-900/95 border border-emerald-500/30 rounded-2xl p-4 shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4 duration-200 text-white">
          <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h4 className="text-xs font-bold text-white tracking-wide">WhatsApp Support</h4>
                <p className="text-[10px] text-emerald-400 font-medium">● Online 24/7 Concierge</p>
              </div>
            </div>
            <button
          ...
          <p className="text-xs text-slate-300 mb-4 leading-relaxed">
            Need instant travel assistance or custom flight deals? Chat with our travel experts on WhatsApp.
          </p>
          ...
          <a
            ...
            className="flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-emerald-500 hover:bg-emerald-600 active:scale-95 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-emerald-500/25"
          >
            <MessageCircle size={16} />
            <span>Chat on {formattedPhone}</span>
          </a>
        </div>
      )}
