          ...
          >
            {activeFlights.slice(0, 6).map((flt: any, i: number) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div><span className="text-xs text-slate-400 block">Fare</span><span className="text-xl font-bold text-amber-400">{flt.farePrice ? `₹${flt.farePrice.toLocaleString("en-IN")}` : "Best Rate"}</span></div>
                  <button onClick={() => { const fareText = flt.farePrice ? `₹${flt.farePrice.toLocaleString("en-IN")}` : "Best Price / On Request"; const msg = `Hi! ✈️ *${flt.airlineName}* — ${flt.fromCity} → ${flt.toCity}\n📌 Class: ${flt.travelClass}\n💰 Fare: ${fareText}\n\nPlease confirm availability.`; openWhatsApp(msg); }} className="px-3 py-2 rounded-2xl bg-green-600 hover:bg-green-500 text-white text-xs font-bold flex items-center gap-1.5 transition-all">
              ...
              activeFlights.map((flt: any, idx: number) => (
                <div key={`flt-m-${idx}`} className="snap-start shrink-0 w-[275px] glass-card p-5 rounded-3xl border border-white/15 bg-slate-900/80 flex flex-col justify-between gap-3 shadow-xl">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <span className="text-lg font-bold text-amber-400">{flt.farePrice ? `₹${flt.farePrice.toLocaleString("en-IN")}` : "Best Rate"}</span>
                    <button onClick={() => { const fareText = flt.farePrice ? `₹${flt.farePrice.toLocaleString("en-IN")}` : "Best Price / On Request"; const msg = `Hi! ✈️ *${flt.airlineName}* ${flt.fromCode}→${flt.toCode}\n📌 ${flt.travelClass}\n💰 ${fareText}\nPlease confirm availability.`; openWhatsApp(msg); }} className="px-3.5 py-2 rounded-xl bg-green-600 hover:bg-green-500 text-white text-xs font-bold flex items-center gap-1">Book</button>
                  </div>
                </div>
              ))
            ) : (
